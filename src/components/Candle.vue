...
export default {
  ...
  created () {
    setTimeout(function(){ this.loading = true; }, 2000);;
  },
  computed: {
    btnKey: {
      get() {
      ...
      set(value) {
        ...
        this.$store.dispatch("apiData", this.id);
      },
    },
  },
  ...
  methods: {
    chartCaller(timeSeriesType) {
      this.CandlestickChart(this.$store.state.apiData, {
        ...
        width: 1000,
        height: 430,
        xDomain: this.$store.state.apiData.map(
          (item) => new Date(item["6. date"])
        ),
        yDomain: [
          this.$store.getters.getLowest,
          this.$store.getters.getHighest,
        ],
    ...
    CandlestickChart(
      ...
      const weekdays = (start, stop) =>
        d3
          ...
          .filter((d) => d.getUTCDay() !== 0 && d.getUTCDay() !== 7);
